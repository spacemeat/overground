...
using namespace overground::config;
...
void overground::config::exportPod(general_t const & src,
...
void overground::config::exportPod(swapchainImageView_t const & src,
humon::HuNode & dest, int depth)
{
  log(0, logTags::warn, "This operation has not been implemented yet.");
...
ostream & overground::config::operator << (ostream & stream, swapchainImageView_t const & rhs)
{
  stream << print(rhs);
  return stream;
}
...
void overground::config::exportPod(swapchain_t const & src,
humon::HuNode & dest, int depth)
{
  log(0, logTags::warn, "This operation has not been implemented yet.");
...
ostream & overground::config::operator << (ostream & stream, swapchain_t const & rhs)
{
  stream << print(rhs);
  return stream;
}
  ...
  humon::HuNode const & src, graphics_t & dest)
{
  ...
  if (src % "bufferStrategy")
  {
    auto & src0 = src / "bufferStrategy";
    BufferStrategy dst0;
    dst0 = fromString<BufferStrategy>((std::string) src0); // leaf
    dest.bufferStrategy = std::move(dst0);
  }
}
...
void overground::config::exportPod(graphics_t const & src,
humon::HuNode & dest, int depth)
{
  log(0, logTags::warn, "This operation has not been implemented yet.");
  ...
  graphics_t const & src, int depth)
{
  ...
  ss << "\n" << indentIn << "bufferStrategy: ";
  ss << to_string(src.bufferStrategy);
  ss << "\n" << prevIndentIn << "}";
  return ss.str();
}

ostream & overground::config::operator << (ostream & stream, graphics_t const & rhs)
{
  stream << print(rhs);
  return stream;
}
...
void overground::config::exportPod(config_t const & src,
humon::HuNode & dest, int depth)
{
  log(0, logTags::warn, "This operation has not been implemented yet.");
...
ostream & overground::config::operator << (ostream & stream, config_t const & rhs)
{
  stream << print(rhs);
  return stream;
}
